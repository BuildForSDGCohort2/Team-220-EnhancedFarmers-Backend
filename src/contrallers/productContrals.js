...
const ProductContraller = {
  ...
  async createProductReadyTosell(req, res) {
    ...
    if (!checkProjectExists.length) {
      return res
        .status(404)
        .send({ status: 404, message: "Project does not exist" });
    }
    ...
    if (!checkFarmerExists.length) {
      return res
        .status(404)
        .send({ status: 404, message: "Farmer does not exist" });
    }

    const createProduct = await ProductModel.createProductForSell(
      product,
      imageUrl
    );
  ...
  async fetchSingleProduct(req, res) {
    ...
    if (!ProductId) {
      return res
        .status(400)
        .status({ status: 400, message: "Id must be provided" });
    }
  ...
  async deleteAproduct(req, res) {
    ...
    return res
      .status(200)
      .send({ status: 200, message: "Product deleted succesffully" });
  },
  ...
  async getProductsWithSpecificCategory(req, res) {
    ...
    if (!productCategory) {
      return res
        .status(400)
        .send({ status: 400, message: "Category is needed" });
    }

    const getThem = await ProductModel.getProductsAccordingToCategory(
      productCategory
    );
