...
const ProjectContrals = {
  ...
  async registerProject(req, res) {
    ...
    const checkFarmerAvailable = await Farmer.findFarmerUsingId(
      project.farmer_id
    );
    if (!checkFarmerAvailable.length) {
      return res
        .status(400)
        .status({ status: 400, message: "Invalid farmer provided" });
    }

    const checkProfesionalAvailable = await Profesional.findProUsingId(
      project.profesional_id
    );
    if (!checkProfesionalAvailable.length) {
      return res
        .status(400)
        .send({ status: 400, message: "Invalid Profesional Provided" });
    }

    const checkInvestorVailable = await Investor.findInvestorUsingId(
      project.investor_id
    );
    if (!checkInvestorVailable.length) {
      return res
        .status(400)
        .send({ status: 400, message: "Invalid Investor Provided" });
    }
  ...
  async getSpecificProject(req, res) {
    const projectId = req.params.id;
    ...
    return res
      .status(200)
      .send({ status: 200, data: checkAvailableProject[0] });
  },
  ...
  async getProjectsSupervisedByTheSameProfessional(req, res) {
    ...
    if (!proId) {
      return res
        .status(404)
        .send({ status: 404, message: "Id must Be Provided" });
    }
